...
async function scrapeSnoop(){
    ...
    for(const truePage of allPages){
        ...
        // Continues onto the next webpage if the current page is invalud
        if(!content){
            continue;
        }

        // Sometimes videos are stores in iframe tags, so we get all the iFrames contained within the page and the source video they're showing, if any
